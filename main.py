...
def matching_AD(pc: str, department_ou: dict) -> str:
    ...
    AD_Return = subprocess.run(["powershell.exe", AD_Query], capture_output=True)
...
def read_registry(pc, registry_key, registry_sz) -> str:
    ...
    cmd = subprocess.run(
        ["powershell.exe", invoke_command], capture_output=True, text=True
    )
...
def run_per_pc(pc) -> None:
    ...
    asset_tag = read_registry(
        pc, BASIC_PATHS["AssetTagKey"], BASIC_PATHS["AssetTag_REG_SZ"]
    )
    if not asset_tag:
    ...
    formatted_text = format_text(pc, asset_tag, contact_string)
    if not formatted_text:
        log.error(f"{pc} FAILED AT FORMATTING TEXT")
        return None

    unc_save_path = local_to_UNC_path(pc, BASIC_PATHS["LocalDestination"])
    if not unc_save_path:
        log.error(f"{pc} FAILED AT UNC PATH CONVERSION")
        return None

    image_source = decide_image_source(department, BASIC_PATHS["ImageSourcePaths"])
    if not image_source:
        log.error(f"{pc} FAILED AT DECIDE IMAGE SOURCE")
        return None

    write_image(image_source, formatted_text, unc_save_path)
